...
powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-amber-model-eval-detail.ps1

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-amber-model-eval-properties.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-amber-model-eval-golden.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

powershell -NoProfile -ExecutionPolicy Bypass -File scripts/check-amber_model_eval_yard-casebook.ps1
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
